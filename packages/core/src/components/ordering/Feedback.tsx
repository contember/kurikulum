...
export interface OrderingFeedbackProps {
  correct?: ComponentChildren
  incorrect?: ComponentChildren
  class?: string
}

export function Feedback({ correct, incorrect, class: className }: OrderingFeedbackProps): VNode | null {
  const ctx = useContext(OrderingContext)
  if (!ctx) throw new Error('Ordering.Feedback must be used within Ordering.Root')
