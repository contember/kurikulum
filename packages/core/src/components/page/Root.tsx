...
export function Root({ id, completion = 'mount', completionTimer, active = true, children, class: className }: PageRootProps): VNode {
  ...
  // Empty children → resolve the page body from the active content bundle.
  // The scroll sentinel is rendered separately, after the body, so it doesn't
  ...
  if (toChildArray(children).length === 0) {
    ...
    if (Body) {
      ...
      body = bundle
        ? (
          <div
            data-kurikulum-missing-page={id}
            style="background:#fee;color:#900;padding:1rem;border-radius:.25rem"
          >
  ...
  return (
    <PageContext.Provider value={{ sentinelRef, id, completion, registry }}>
      ...
      >
        ...
        {completion === 'scroll' && <div ref={sentinelRef} aria-hidden="true" />}
      </main>
