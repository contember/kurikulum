...
export function FillBlank({ id, question, accept, caseSensitive, weight, placeholder, children }: FillBlankProps): VNode {
  return (
    <FB.Root id={id} accept={accept} caseSensitive={caseSensitive} weight={weight} aria-label={question}>
      <FB.Label class="font-semibold mb-2">{question}</FB.Label>
      <FB.Input
      ...
      />
      {children}
      <FB.Submit class="mt-2 px-4 py-2 bg-primary text-white rounded-default hover:bg-primary-hover disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2">
        Odeslat
      </FB.Submit>
    </FB.Root>
  )
